...
export default function KPICard({ title, value, subtitle, icon, trend, color = 'brand', loading = false }) {
  ...
  return (
    <motion.div
      ...
      className={clsx(
        'relative overflow-hidden bg-gradient-to-br border rounded-2xl p-5',
        colors[color]
      )}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <p className="text-xs text-muted-400 uppercase tracking-wider font-medium mb-1">{title}</p>
          {loading
            ? <div className="h-7 w-24 bg-surface-400 rounded animate-pulse" />
            : <p className="text-2xl font-bold text-white truncate">{value}</p>
          }
          {subtitle && <p className="text-xs text-muted-400 mt-1">{subtitle}</p>}
        </div>
        {icon && (
          <div className={clsx('p-2.5 rounded-xl shrink-0', iconColors[color])}>
            {icon}
          </div>
        )}
      </div>
      {trend !== undefined && (
        <div className={clsx('flex items-center gap-1 mt-3 text-xs font-medium', trend >= 0 ? 'text-success-400' : 'text-danger-400')}>
